...
include 'config.php';
session_start();
$q=mysql_query("SELECT q3 FROM flag WHERE email='".$_SESSION['email']."'");
...
$up =mysql_query("UPDATE `flag` SET `q3` = '1' WHERE `flag`.`Email` = '".$_SESSION['email']."'");}
}
$q=mysql_query("SELECT q3 FROM flag WHERE email='".$_SESSION['email']."'");
$que12=mysql_query("SELECT count FROM applicants WHERE email='".$_SESSION['email']."'");
if($que12){
...
$answer=mysql_query("SELECT q3 FROM `applicants` WHERE email='".$_SESSION['email']."'");
...
<h3>Question 3: Program to print weekday of given date.</h3>
<form class="" action="q3.php" method="post">
  <div class="form-group">
    <label for="txtarea1" class="col-sm-2 control-label"><h3>Answer</h3></label>
    <div class="col-sm-8"><textarea name="txtarea3" id="txtarea3" cols="50" rows="100000000" class="form-control1" style="height:300px"><?php echo $ans2[0]; ?></textarea></div>
  </div>
